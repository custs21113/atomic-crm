...
export const Note = ({
  ...
  const [deleteNote] = useDelete(resource, undefined, {
    ...
    onSuccess: () => {
      notify("crm.notes.deleted", { type: "info", undoable: true });
    },
  });
        ...
        <span className={`${isHover ? "visible" : "invisible"}`}>
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent>
                <p>{translate("crm.notes.delete")}</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </span>
      ...
      {isEditing ? (
        ...
        <div className="pt-2 text-sm max-w-150">
          ...
          {isTruncated && (
            ...
            >
              {isExpanded ? translate("crm.notes.show_less") : translate("crm.notes.read_more")}
            </button>
          )}
